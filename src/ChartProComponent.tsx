...
const ChartProComponent: Component<ChartProComponentProps> = (props) => {
  ...
  onMount(() => {
    ...
    widget = init(widgetRef!, {
      customApi: {
        formatBigNumber: function toLocaleString(
          value: number | string,
          locales?: string
        ) {
          ...
          if (Math.abs(num) < 0.00000001 && num !== 0) {
            ...
            return str.replace(/(\.0{7,})(\d+)/, (match, zeros, digits) => {
              const count = zeros.length - 1;
              const subscript = count
                .toString()
                .split("")
                .map((d) => String.fromCodePoint(0x2080 + parseInt(d, 10)))
                .join("");
              const validDigits = digits.slice(0, 4);
              return ".0" + subscript + validDigits;
            });
          }
